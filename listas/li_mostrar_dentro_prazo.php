...
<table id="mostrar" class="hover">
    ...
    try {
        ...
        try{
            $obj_mostrar = new Crud();
            $obj_mostrar->setTabelas("tbl_antivirus_pc LEFT JOIN tbl_usuario_computador");
            $dados_mostra = $obj_mostrar->listar();
            $lista_mostra = new ArrayIterator($dados_mostra);
        }catch (Exception $e){
        ...
        $tabela = "tbl_antivirus_pc LEFT JOIN tbl_usuario_computador";
        $condicao_on = "apc_serial_pc = uc_serial";
        $clausula_where = "(apc_validade-DATEDIFF(CURDATE(), apc_data_registo)) > 0 ORDER BY apc_data_vencimento ASC";

        $mostra_permanente = new Crud();
        if ($mostra_permanente->numeroDeRegistos_permanente_r_a_pc($tabela, $condicao_on, $clausula_where)) {
            $dados = $mostra_permanente->mostra_permanente_r_a_pc($tabela, $condicao_on, $clausula_where);
